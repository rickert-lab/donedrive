...
>[!IMPORTANT]
>User access could be delegated to applications like _donedrive_ with user authentication: However, app access must always be authorized by every admin of every SharePoint installation. Therefore, _donedrive_ only accepts share links without user authentication ("The Link works for Anyone") and without passwords:
